...
func NewUpstreamManager(upstreams []string) *Manager {
	um := &Manager{MaxAttempts: DefaultMaxAttempts}
	if len(upstreams) == 0 {
...
func (um *Manager) DoDeadline(req *fasthttp.Request, resp *fasthttp.Response, deadline time.Time) error {
...
func (um *Manager) DoTimeout(req *fasthttp.Request, resp *fasthttp.Response, timeout time.Duration) error {
	deadline := time.Now().Add(timeout)
	return retry(req, resp, deadline, um.get().DoDeadline, um.maxAttempts)
}

func (um *Manager) setMaxAttempts() {
...
func (um *Manager) Do(req *fasthttp.Request, resp *fasthttp.Response) error {
	timeout := um.Timeout
	if timeout <= 0 {
